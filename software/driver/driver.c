...
OSStatus doEWrite(driverGlobalsPtr theGlobals, EParamBlkPtr pb) {
  ...
  totalLength = 4; /* Add 4 bytes to frame length to account for FCS field
                      generated by ethernet controller */
  do {
    totalLength += wds->entryLength;
    wds++;
  } while (wds->entryLength);

  /* Block transmission of oversized frames. ENC624J600 will automatically
  append padding bytes to undersized frames. */
  if (totalLength > 1518) {
    return eLenErr;
  }
